...
class wavelet2D {
 public:
  ...
  void inverseTransformS(const std::shared_ptr<float2DReg> in,
                         std::shared_ptr<float2DReg> out);
  void forwardTransformVP(const std::shared_ptr<float2DReg> in,
                          std::shared_ptr<float2DReg> out);
  void inverseTransformVP(const std::shared_ptr<float2DReg> in,
                          std::shared_ptr<float2DReg> out);
  void forwardTransformV(const std::shared_ptr<float2DReg> in,
                         std::shared_ptr<float2DReg> out);
  void inverseTransformV(const std::shared_ptr<float2DReg> in,
                         std::shared_ptr<float2DReg> out);
